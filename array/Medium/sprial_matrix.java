...
public class sprial_matrix {
    public static void main(String[] args) {
        ...
        int bottom=matrix.length-1; //to keep count of bottom row
        int left=0; //to keep left element
        int right=matrix[0].length-1; //to keep right element
        ...
        while(top<=bottom && left<=right){ //either top <= bottom and left<=right
            for(int i=left;i<=right;i++){
                result.add(matrix[top][i]); //add first row
            }
            ...
            if(top<=bottom){
                for(int i=right;i>=left;i--){
                    result.add(matrix[bottom][i]);//to add botttom row from n-1;
                }
                bottom--;
            }
            if(left<=right){
                for(int i=bottom;i>=top;i--){
                    result.add(matrix[i][left]); //from left to right
                }
                left++;
            }
        }
